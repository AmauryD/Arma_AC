...
_parent = param [1,findDisplay 46,[displayNull]];
_headerText = param [0,"Enter Text",[""]];

if(!canSuspend) exitWith {["edit dialog doesn't allow this context to execute"] call ac_fnc_debugMessage};
if(!isNull findDisplay 1234567) exitWith {["Enter text dialog already openend"] call ac_fnc_debugMessage};
