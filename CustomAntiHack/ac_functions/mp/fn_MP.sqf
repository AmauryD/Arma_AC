...
with missionnamespace do {
	...
	//--- Send to server
	AC_fnc_MP_packet = [0,_params,_functionName,_target,_isPersistent,_isCall];
	publicvariableserver "AC_fnc_MP_packet";

	//--- Local execution
	if !(ismultiplayer) then {
		["AC_fnc_MP_packet",AC_fnc_MP_packet] spawn ac_fnc_MPexec;
	};
